...
// migration that was a live scrape; it is now DynamoDB. Every behaviour
// asserted here predates the migration — only the backing store changed.
...
describe('listingResolver', () => {
  ...
  it('resolves a listing present in the active index without a point lookup', async () => {
    vi.mocked(queryRecent).mockResolvedValue([makeListing()]);
  ...
  it('resolves when id differs from appUrl', async () => {
    vi.mocked(queryRecent).mockResolvedValue([makeListing({ id: 'custom-id-123' })]);
  ...
  it('falls back to a point lookup when the read throws', async () => {
    vi.mocked(queryRecent).mockRejectedValue(new Error('network down'));
  ...
  it('returns null for empty input without reading', async () => {
    ...
    await expect(resolveInternship('   ')).resolves.toBeNull();
    expect(queryRecent).not.toHaveBeenCalled();
  });
  ...
  it('caches the index so repeated lookups read once', async () => {
    ...
    expect(queryRecent).toHaveBeenCalledTimes(1);
  });
  ...
  it('re-reads after the cache is cleared', async () => {
    ...
    expect(queryRecent).toHaveBeenCalledTimes(2);
  });
  ...
  it('does not fail the resolve when the store is empty', async () => {
    ...
    expect(vi.mocked(queryRecent).mock.calls[0][0]).toMatchObject({
      throwOnEmpty: false,
    });
  });
});
